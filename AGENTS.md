...
- **加载器持久化的条目也会引用已消失的包并炸掉启动**。皮肤中心等功能在运行时把选中的包 pnpm 装进 profile 并把条目写进 `~/.dsh/profiles/web/cordis.yml`；换 flavor 或本地安装损坏后该包解析不到，dsh 报 ERR_MODULE_NOT_FOUND 拒绝启动。不能安全地改用户配置——healUnresolvableEntries 的做法是给解析不到的条目在 profile node_modules 放一个无操作占位包（带 `.dsh-desktop-stub` 标记），当前运行时能提供真包时占位自动退位，真实重装会直接覆盖它。
