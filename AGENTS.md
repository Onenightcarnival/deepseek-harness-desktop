...
node update-locks.mjs <dsh版本> ["插件@版本"...]    # 升级 dsh：不做实时解析，把上一份 full 锁
                                                      #   整体平移到目标版本并重写两份锁（见坑清单）
node stage-dsh.mjs --update-locks                     # 兜底的实时解析路径（可能指数回溯，优先用上面的脚本）
...
- **staging 不做实时 npm 解析，装锁文件**。`npm install @deepseek-ai/dsh@latest` 的依赖图会让 arborist 的 peer 回溯偶发指数爆炸——注册表侧一次无关发版就能把 30 秒的安装变成 mac runner 上的 OOM（2GB 堆，SIGABRT；linux 上同样复现，10 分钟不出结果）。所以 stage-dsh.mjs 默认 `npm ci --force` 装 `locks/<flavor>.package-lock.json`（一份锁经 os/cpu 条件项覆盖全平台，带完整性校验，20-30 秒；`--force` 是因为 npm ci 还会复验 peer——预置插件落后 dsh 一个小版本时会拒装，而锁本身就是决策记录，真实兼容性由 staging 冒烟验证）。**升级 dsh 用 `update-locks.mjs`，不要实时解析**：它把上一份 full 锁按 lockstep 整体平移到目标版本（重刷 resolved/integrity、递归补齐新引用的包、放宽 npm ci 校验不过的 peer 区间、报告依赖形状漂移），秒级完成；预置插件的**增删换**也走它——移除的插件子树按解析语义剪枝（不可达条目删除），新增包按引用方的 semver 区间取版本（不是 latest——mongodb 要 ^6 装成 7.x 会被 npm ci 拒），**可选 peer 不递归拉入**（npm 不自动装它们；跟着 mongodb 的 kerberos/b4a 的 react-native 递归一跳曾误加 395 个包）；注意树里已存在的名字若不满足某个可选 peer 区间，npm ci 同样报 Invalid，pass 3 的放宽对可选 peer 一并生效。**内核跨线升级（0.1.1-rc.2 → 0.1.2-rc.1）暴露的三个必要环节**：(a) 非 lockstep 的支撑包要按引用方区间**真提版**而不是放宽——rc.1 把 `@deepseek-ai/cordis` peer 提到 ^4.0.2，只放宽会让新内核跑在旧 cordis 上（pass 2b：所有引用方区间的交集里取最高版）；(b) 引用方区间与顶层版本不可调和时要**嵌套私有拷贝**（pass 2c：compression 要 debug ^2.6 而树里是 4.x，npm ci 报 "Missing: debug@2.6.9"）；(c) 剪枝要**在**这些 pass 之前先跑一遍且不沿可选 peer 走——better-locale@0.1.0 作为可选 peer 曾把 rc.1 已删除的 dsh-client-runtime/host-apiproxy（rc.2 版）连同 14 个 rc.2 core 包留在闭包里。better-locale 现在走 plugins-full.json 的 carry 组显式钉版（bs 声明的 peer 区间 ^0.1.0 早已落后其实际发布线）——0.1.1-rc.2 升级实测 192 个包平移零漂移、新增 1 个真实新包 dsh-authorization。实时解析的教训：`--legacy-peer-deps` 会跳过 peer 自动安装，产出的锁缺 118 个核心包（dsh-shell/fs/sandbox…），绝不能用；`--force` 实时解析 26 分钟不出结果。锁根依赖与插件清单不一致时 stage 直接报错提示重新生成。验证预置插件在真机挂载：0.1.2-rc.1 起客户端 bundle 只经**组合路由**下发——先用就绪行里带 token 的 URL 换 cookie（303），再取首页 HTML 里 `href="/plugins/??<包1>/client.js,<包2>/client.js,…&rev=<hash>"` 那条精确组合 URL 拉 bundle，断言其中含 `id: "<包名>"` 的模块（单包 `/plugins/<包名>/client.js` 与自拼的组合都 404，rc2 时代的单包探针已失效）。
...
- **0.1.2-rc.1 起就绪行带一次性 token**（`dsh web: http://127.0.0.1:<port>/?token=…`），裸 origin 回 401，`/api` 也受 browser-trust 围栏保护。READY_RE 必须捕获整条 URL（含 query）并原样 loadURL——窗口内完成 303 换 cookie；每次启动 token 都不同，原地重启服务同样拿新 URL。CLI 用法也变成 `dsh --profile web`（子命令形态 `dsh web` 仍被接受）。
- **应用内内核升级只允许同版本线**（runtime.js `releaseLine`：去掉预发布标签的 major.minor.patch）。第三方插件按线适配——0.1.2 阵营（bs 0.18、dsh-web 0.3.7+）直接删掉了 0.1.1-rc.x 兼容层——而 installCoreRuntime 装的是包内钉死的预置版本，跨线一点就是"新内核 + 旧阵营插件"必炸的组合。跨线时静默检查不打扰，手动检查引导去下载新安装包。
