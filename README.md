...
- **检查内核更新…**：查询 npm 上的 `@deepseek-ai/dsh`，一键升级到用户数据目录的 `runtimes/<版本>/`，重启生效；升级失败自动回退到内置版本。只在同一版本线内升级（如 0.1.5-rc.1 → rc.2），跨线需下载新安装包。「帮助」菜单第一项显示当前内核版本。
...
产物在 `dist/`。开发调试：staging 后 `npm start`。升级 Electron 需满足 dsh 的 engines 要求（当前 `^22.19.0 || >=24.0.0`；Electron 43 内置 Node 24）。
