...
- 菜单「插件 → 配置中心…」左侧导航分四页：**插件**（按 npm 包名/来源
  ...
  风格三种模式：不使用代理 / 使用系统代理 / 手动配置。系统代理经
  Chromium 解析操作系统设置（支持 PAC）；手动配置填主机名 + 端口、
  不代理的主机列表、可选身份验证（用户名/密码，可选记住——不记住则
  密码仅保存在本次运行中）、TLS 选项（默认信任系统证书库；可导入
  代理的 CA 证书；兜底的"不校验证书"开关，用于公司代理做 TLS 拦截
  的场景）。对 dsh 服务及其全部网络请求生效——模型 API、插件安装、
  内核升级、MCP 连接；本机地址始终直连；带「测试连通」。保存后
  自动原地重启 dsh 服务即时生效（窗口短暂回到加载页），无需重启
  应用）。
