...
- 菜单「插件 → 配置中心…」提供三个标签：**插件**（按 npm 包名/来源
  安装、移除，装到用户配置层）、**MCP 服务器**（表单增删 stdio /
  streamable-http 服务器，配置写入用户配置层的标记托管区块，
  保存即热生效无需重启，不碰用户手写条目；本机没有 npx，stdio 命令用 `pnpm dlx 包名` 或绝对
  路径）、**技能**（安装 zip 技能包——单技能或多技能合集均可，列表删除、
  打开目录，装删即时生效无需重启；目录固定为 dsh 约定的
  `~/.dsh/skills`）。
