...
[dsh-web-ui](https://github.com/zhu1090093659/dsh-web-ui) 的六个精选
插件：任务看板、Git 图谱、右侧预览面板、鲸鱼娘宠物、皮肤中心、SSH
远程连接，首次启动自动挂载进用户配置层，之后可像普通插件一样在
配置中心逐个移除（移除后不会被重新塞回来）。十款皮肤随包携带但
不自动启用——在皮肤中心里选择启用（同时只生效一款），离线可用。两种版本共享 `~/.dsh`
