...
- **技能**：安装 zip 技能包（单技能或多技能合集）。列表按 SKILL.md 的 frontmatter 展示名称、版本、描述与调用面；每项有启用开关和删除。点名称进详情：全部 frontmatter 字段、源文件树、只读预览（文本限 256 KB）、打开目录。关闭的技能移入 `~/.dsh/disabled_skills/`，再打开即移回。安装、删除、开关即时生效。
