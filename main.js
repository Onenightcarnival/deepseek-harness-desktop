...
function desktopPatchArgs() {
  ...
  const args = p ? ['--patch', p] : []
  // GUI-managed MCP servers (配置中心 → MCP 标签) live in their own overlay.
  const mcp = mcpPatchPath()
  if (fs.existsSync(mcp)) args.push('--patch', mcp)
  return args
}
...
function mcpPatchPath() { return path.join(app.getPath('userData'), 'mcp-patch.yml') }

function readMcpServers() {
  try { return JSON.parse(fs.readFileSync(mcpStorePath(), 'utf8')) } catch { return [] }
}

/** Render the server list as a cordis patch overlay (one dsh-mcp-client per server). */
function writeMcpPatch(servers) {
  const q = (s) => JSON.stringify(String(s)) // YAML double-quoted scalar
  const lines = ['# GENERATED by DeepSeek Harness Desktop（配置中心 → MCP）— 手改会被覆盖']
  if (servers.length === 0) {
    lines.push('[]')
  } else {
    lines.push('- insert:')
  ...
  fs.writeFileSync(mcpPatchPath(), lines.join('\n') + '\n')
}
